...
describe("CardView", () => {
  ...
  function makeCardView(merchantConfiguration = {}) {
    ...
    cardElement = document.body.querySelector(
      ".braintree-sheet.braintree-card"
    );
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(hostedFields, "create")
      .mockResolvedValue(fakeHostedFieldsInstance);
  ...
  describe("initialize", () => {
    ...
    test("has cvv if supplied in challenges", () => {
      ...
      return view.initialize().then(() => {
        expect(
          cardElement.querySelector('[data-braintree-id="cvv-field-group"]')
        ).toBeDefined();
      });
    });

    test("does not have cvv if supplied in challenges, but hosted fields overrides sets cvv to null", () => {
      fakeClient.getConfiguration.mockReturnValue({
        gatewayConfiguration: {
          challenges: ["cvv"],
          creditCards: {
    ...
    test("does not have postal code if supplied in challenges, but hosted fields overrides sets postal code to null", () => {
      fakeClient.getConfiguration.mockReturnValue({
        gatewayConfiguration: {
          challenges: ["postal_code"],
          creditCards: {
    ...
    test("does not include cardholderName if not provided in merchant configuration", () => {
      fakeModel.merchantConfiguration.card = {};
    ...
    test("does not remove hidden class from save card input if not configured", () => {
      fakeModel.merchantConfiguration.card = {};
      fakeModel.isGuestCheckout = false;
    ...
    test("starts async dependency", () => {
      ...
      return view.initialize().then(() => {
        expect(DropinModel.prototype.asyncDependencyStarting).toBeCalledTimes(
          1
        );
      });
    });

    test("notifies async dependency is ready when Hosted Fields is created", () => {
    ...
    test("creates Hosted Fields with number and expiration date", () => {
      ...
      return view.initialize().then(() => {
        expect(hostedFields.create).toBeCalledWith(
        ...
        expect(hostedFields.create.mock.calls[0][0]).not.toHaveProperty(
          "fields.cvv"
        );
        expect(hostedFields.create.mock.calls[0][0]).not.toHaveProperty(
          "fields.postalCode"
    ...
    test("creates Hosted Fields with cvv if included in challenges", () => {
      ...
      return view.initialize().then(() => {
        expect(hostedFields.create.mock.calls[0][0].fields).toHaveProperty(
          "cvv"
        );
      });
    });

    test("creates Hosted Fields with postal code if included in challenges", () => {
      fakeClient.getConfiguration.mockReturnValue({
        gatewayConfiguration: {
          challenges: ["postal_code"],
          creditCards: {
    ...
    test("calls asyncDependencyFailed with an error when Hosted Fields creation fails", () => {
      const fakeError = {
    ...
    test("shows supported card icons", () => {
      ...
      return view.initialize().then(() => {
        supportedCardTypes.forEach((cardType) => {
          const cardIcon = cardElement.querySelector(
            '[data-braintree-id="' + cardType + '-card-icon"]'
          );
    ...
    test("does not show UnionPay icon even if it is supported", () => {
      ...
      return view.initialize().then(() => {
        unionPayCardIcon = cardElement.querySelector(
          '[data-braintree-id="unionpay-card-icon"]'
        );

        expect(unionPayCardIcon.classList.contains("braintree-hidden")).toBe(
          true
        );
      });
    });
    ...
    test("sets field placeholders", () => {
      ...
      return view.initialize().then(() => {
        hostedFieldsConfiguredFields =
          hostedFields.create.mock.calls[0][0].fields;

        expect(hostedFieldsConfiguredFields.number.placeholder).toBe(
          "•••• •••• •••• ••••"
        );
        expect(hostedFieldsConfiguredFields.expirationDate.placeholder).toBe(
          strings.expirationDatePlaceholder
        );
    ...
    test("does not add hosted fields elements for fields that are not present", () => {
      let hostedFieldsConfiguredFields;
    ...
    test("ignores changes to selector in field options", () => {
      ...
      return view.initialize().then(() => {
        const hostedFieldsConfiguredFields =
          hostedFields.create.mock.calls[0][0].fields;

        expect(hostedFieldsConfiguredFields.number.selector).not.toBe(
          "#some-selector"
        );
      });
    });
    ...
    test("allows overriding styles options for hosted fields", () => {
      ...
      return view.initialize().then(() => {
        hostedFieldsConfiguredStyles =
          hostedFields.create.mock.calls[0][0].styles;
        ...
        expect(hostedFieldsConfiguredStyles.input["font-family"]).toBe(
          "fantasy"
        );
    ...
    test("allows overriding styles options with class name for hosted fields", () => {
      let hostedFieldsConfiguredStyles;
  ...
  describe("isEnabled", () => {
    ...
    test("resovles with true when there is at least one supported card type", () => {
    ...
    test("resovles with false when merchant configuration sets card to false", () => {
      const configuration = fake.configuration();
    ...
    test("resovles with false when there are no supported card types", () => {
      const configuration = fake.configuration();
  ...
  describe("requestPaymentMethod", () => {
    ...
    test("calls the callback with the payload when tokenize is successful", () => {
      const cardView = new CardView({
        element: cardElement,
        model: fakeModel,
        client: fakeClient,
  ...
  describe("Hosted Fields events", () => {
    ...
    describe("onFocusEvent", () => {
      ...
      test("emits a focus event on the model", () => {
        return cardView.initialize().then(() => {
          expect(cardView.model._emit).toBeCalledWith(
            "card:focus",
            eventPayload
          );
        });
      ...
      test("shows default card icon in number field when focused", () => {
        return cardView.initialize().then(() => {
          const cardNumberIcon = cardElement.querySelector(
            '[data-braintree-id="card-number-icon"]'
          );

          expect(cardNumberIcon.classList.contains("braintree-hidden")).toBe(
            false
          );
          expect(
            cardNumberIcon.querySelector("use").getAttribute("xlink:href")
          ).toBe("#iconCardFront");
        });
      });
      ...
      test("shows default cvv icon in cvv field when focused", () => {
        ...
        return cardView.initialize().then(() => {
          const cvvIcon = cardElement.querySelector(
            '[data-braintree-id="cvv-icon"]'
          );

          expect(cvvIcon.classList.contains("braintree-hidden")).toBe(false);
          expect(cvvIcon.querySelector("use").getAttribute("xlink:href")).toBe(
            "#iconCVVBack"
          );
        });
      });
      ...
      test("adds braintree-form__field-group--is-focused", () => {
        ...
        const numberFieldGroup = cardElement.querySelector(
        ...
        return cardView.initialize().then(() => {
          expect(
            numberFieldGroup.classList.contains(
              "braintree-form__field-group--is-focused"
            )
          ).toBe(true);
        });
    ...
    describe("onBlurEvent", () => {
      ...
      test("removes braintree-form__field-group--is-focused class when blurred", () => {
        const numberFieldGroup = cardElement.querySelector(
          '[data-braintree-id="number-field-group"]'
        );
      ...
      test("applies error class if field is not valid", () => {
        const numberFieldError = cardElement.querySelector(
        ...
        return cardView.initialize().then(() => {
          ...
          expect(numberFieldError.textContent).toBe(
            "This card number is not valid."
          );
        });
      });

      test("does apply error class if field is empty when focusing another hosted field", () => {
      ...
      test("sets the empty error when programatically focussing a hosted field (requires a setTimeout)", (done) => {
      ...
      test("does not apply error class if field is empty and not focusing hosted fields", () => {
      ...
      test("does not apply error class if field is empty and the active element is not an iframe", () => {
        const fakeElement = document.createElement("div");
    ...
    describe("onCardTypeChange event", () => {
      ...
      test("adds the card-type-known class when there is one possible card type", () => {
        const numberFieldGroup = cardElement.querySelector(
          '[data-braintree-id="number-field-group"]'
        );
        ...
        return cardView.initialize().then(() => {
          expect(
            numberFieldGroup.classList.contains(
              "braintree-form__field-group--card-type-known"
            )
          ).toBe(true);
        });
      });

      test("removes the card-type-known class when there is no possible card type", () => {
        const numberFieldGroup = cardElement.querySelector(
          '[data-braintree-id="number-field-group"]'
        );
      ...
      test("removes the card-type-known class when there are many possible card types", () => {
        const numberFieldGroup = cardElement.querySelector(
          '[data-braintree-id="number-field-group"]'
        );
      ...
      test("updates the card number icon to the card type if there is one possible card type", () => {
        const cardNumberIcon = cardElement.querySelector(
          '[data-braintree-id="card-number-icon"]'
        );
        ...
        return cardView.initialize().then(() => {
          expect(
            cardNumberIcon.querySelector("use").getAttribute("xlink:href")
          ).toBe("#icon-master-card");
        });
      });

      test("updates the card number icon to the generic card if there are many possible card types", () => {
        const cardNumberIcon = cardElement.querySelector(
          '[data-braintree-id="card-number-icon"]'
        );
      ...
      test("updates the card icon to the generic card if there no card types", () => {
        const cardNumberIcon = cardElement.querySelector(
          '[data-braintree-id="card-number-icon"]'
        );
      ...
      test("updates the cvv icon to back icon for non-amex cards", () => {
        const use = cardElement
          .querySelector('[data-braintree-id="cvv-icon"]')
          .querySelector("use");
      ...
      test("updates the cvv label descriptor to four digits when card type is amex", () => {
        const cvvLabelDescriptor = cardElement
          .querySelector('[data-braintree-id="cvv-field-group"]')
          .querySelector(".braintree-form__descriptor");
      ...
      test("updates the cvv label descriptor to three digits when card type is non-amex", () => {
        const cvvLabelDescriptor = cardElement
          .querySelector('[data-braintree-id="cvv-field-group"]')
          .querySelector(".braintree-form__descriptor");
      ...
      test("updates the cvv label descriptor to three digits when multiple card types", () => {
        const cvvLabelDescriptor = cardElement
          .querySelector('[data-braintree-id="cvv-field-group"]')
          .querySelector(".braintree-form__descriptor");
      ...
      test("updates the cvv field placeholder when card type is non-amex", () => {
        eventPayload = {
          cards: [{ type: "visa" }],
          emittedBy: "number",
        };
      ...
      test("updates the cvv field placeholder when multiple card types", () => {
        eventPayload = {
          cards: [{ type: "american-express" }, { type: "visa" }],
          emittedBy: "number",
        };
      ...
      test("does not update the cvv field placeholder when there is no cvv challenge", () => {
        eventPayload = {
          cards: [{ type: "american-express" }, { type: "visa" }],
          emittedBy: "number",
        };
      ...
      test("does not update the cvv field placeholder when it is removed with an override", () => {
        eventPayload = {
          cards: [{ type: "american-express" }, { type: "visa" }],
          emittedBy: "number",
        };
      ...
      test("does not update the cvv field placeholder when using a custom CVV placeholder", () => {
        eventPayload = {
          cards: [{ type: "american-express" }, { type: "visa" }],
          emittedBy: "number",
        };
    ...
    describe("onValidityChangeEvent", () => {
      ...
      test("removes the braintree-form__field-group--has-error class if a field is potentially valid", () => {
        const numberFieldGroup = cardElement.querySelector(
          '[data-braintree-id="number-field-group"]'
        );
      ...
      test("adds braintree-form__field--valid class to valid expiration date field", () => {
        const expirationElement = cardElement.querySelector(
        ...
        return cardView.initialize().then(() => {
          expect(
            expirationElement.classList.contains("braintree-form__field--valid")
          ).toBe(true);
        });
      });

      test("removes braintree-form__field--valid class to invalid expiration date field", () => {
        const expirationElement = cardElement.querySelector(
          ".braintree-form-expiration"
        );
      ...
      test("adds braintree-form__field--valid class to valid number with card type supported", () => {
        const numberElement = cardElement.querySelector(
        ...
        return cardView.initialize().then(() => {
          expect(
            numberElement.classList.contains("braintree-form__field--valid")
          ).toBe(true);
        });
      });

      test("removes braintree-form__field--valid class to valid number without card type supported", () => {
        const numberElement = cardElement.querySelector(
          ".braintree-form-number"
        );
      ...
      test("removes braintree-form__field--valid class to not valid number with card type supported", () => {
        const numberElement = cardElement.querySelector(
          ".braintree-form-number"
        );
      ...
      test("calls model.setPaymentMethodRequestable with isRequestable true if form is valid", () => {
        const numberElement = cardElement.querySelector(
          ".braintree-form-number"
        );
        ...
        jest
          .spyOn(cardView.model, "setPaymentMethodRequestable")
          .mockImplementation();
      ...
      test("calls model.setPaymentMethodRequestable with isRequestable false if form is invalid", () => {
        const numberElement = cardElement.querySelector(
          ".braintree-form-number"
        );
      ...
      test("does not call model.setPaymentMethodRequestable if tokenization is in progress", () => {
        const numberElement = cardElement.querySelector(
          ".braintree-form-number"
        );
    ...
    describe("passthrough events", () => {
      ...
      test.each(["empty", "inputSubmitRequest", "binAvailable"])(
        "passes along %s event",
        (eventName) => {
  ...
  describe("tokenize", () => {
    ...
    beforeEach(() => {
      ...
      jest
        .spyOn(transitionHelper, "onTransitionEnd")
        .mockImplementation(yields());
    ...
    test("calls callback with error and reports error to DropinModel if form is not valid", () => {
      expect.assertions(3);
      ...
      return cardView.tokenize().catch((err) => {
        expect(fakeHostedFieldsInstance.tokenize).not.toBeCalled();
        expect(cardView.model.reportError).toBeCalledWith(
    ...
    test("calls callback with error when cardholder name is required and the input is empty", () => {
      ...
      return makeCardView({
        ...
        .then((view) => {
          ...
          jest
            .spyOn(cardViewWithCardholderName.model, "reportError")
            .mockImplementation();
        ...
        .catch((err) => {
          expect(fakeHostedFieldsInstance.tokenize).not.toBeCalled();
          expect(cardViewWithCardholderName.model.reportError).toBeCalledWith(
            "hostedFieldsFieldsInvalidError"
          );
          expect(err.message).toBe("No payment method is available.");
        });
    ...
    test("does not error if cardholder name is empty, but not required", () => {
      let cardViewWithCardholderName;
    ...
    test("does not error if cardholder name is not included", () => {
      ...
      return makeCardView({
        ...
        .then((view) => {
          ...
          jest
            .spyOn(cardViewWithoutCardholderName.model, "reportError")
            .mockImplementation();
        ...
        .then(() => {
          expect(
            cardViewWithoutCardholderName.model.reportError
          ).not.toBeCalled();
          expect(fakeHostedFieldsInstance.tokenize).toBeCalledTimes(1);
        });
    });

    test("calls callback with error when cardholder name length is over 255 characters", () => {
      let cardViewWithCardholderName;
    ...
    test("reports an error to DropinModel when Hosted Fields tokenization returns an error", () => {
      expect.assertions(1);
    ...
    test("reports a duplicate card error to DropinModel when tokenization returns an error", () => {
      expect.assertions(1);
    ...
    test("shows unsupported card field error when attempting to use an unsupported card and reports an error", () => {
      expect.assertions(4);
      ...
      return cardView.tokenize().catch(() => {
        expect(numberFieldError.classList.contains("braintree-hidden")).toBe(
          false
        );
        expect(numberFieldError.textContent).toBe(
    ...
    test("shows empty field error when attempting to sumbit an empty field", () => {
      expect.assertions(3);
    ...
    test("shows invalid field error when attempting to submit an invalid field", () => {
      expect.assertions(3);
    ...
    test("sets the aria-invalid attribute and set message when a field error is shown", () => {
      fakeHostedFieldsInstance.getState.mockReturnValue({
        cards: [{ type: "visa" }],
        fields: {
          number: {
    ...
    test("sets the aria-invalid attribute on an input when a field error is hidden", () => {
      const input = {
        id: {
          indexOf: function () {
            return 1;
    ...
    test("removes the aria-invalid attribute and message when a field error is hidden", () => {
      fakeHostedFieldsInstance.getState.mockReturnValue({
        cards: [{ type: "visa" }],
        fields: {
          number: {
    ...
    test("removes the aria-invalid attribute on an input when a field error is hidden", () => {
      const input = {
        id: {
          indexOf: function () {
            return 1;
    ...
    test("includes `vaulted: true` in tokenization payload if not guest checkout", () => {
    ...
    test("does not include `vaulted: true` in tokenization payload if save card input is not checked", () => {
      cardView.model.isGuestCheckout = false;
    ...
    test("does not include `vaulted: true` in tokenization payload if guest checkout", () => {
    ...
    test("does not call hostedFieldsInstance.tokenize if form is invalid", () => {
      expect.assertions(1);
    ...
    test("does not call hostedFieldsInstance.tokenize if form is valid, but card type is not supported", () => {
      expect.assertions(1);
    ...
    test("does not call hostedFieldsInstance.tokenize if form is valid, but no card is available in state", () => {
      expect.assertions(1);
    ...
    test("vaults on tokenization if not using guest checkout", () => {
      ...
      return cardView.tokenize().then(() => {
        expect(fakeHostedFieldsInstance.tokenize).toBeCalledWith({
          vault: true,
        });
      });
    });

    test("does not vault on tokenization if save card input is not checked", () => {
      cardView.model.isGuestCheckout = false;
      cardView.saveCardInput.checked = false;
    ...
    test("clears cardholder name field if it exists after successful tokenization", () => {
      let cardViewWithCardholderName;
    ...
    test("does not clear fields after successful tokenization if merchant configuration includes clearFieldsAfterTokenization as false", () => {
    ...
    test("does not clear cardholder name field after successful tokenization if merchant configuration includes clearFieldsAfterTokenization as false", () => {
      let cardViewWithCardholderName;
    ...
    test("removes braintree-sheet--loading class after successful tokenization", (done) => {
      const stubPayload = {};
      ...
      cardView.tokenize().then(() => {
        setTimeout(() => {
          expect(classList.remove).toBeCalledWith(
            cardElement,
    ...
    test("removes braintree-sheet--loading class after tokenization fails", () => {
      expect.assertions(1);
    ...
    test("adds a new payment method when tokenize is successful and transition ends", () => {
      const stubPayload = {};
    ...
    test("does not update the active payment method when tokenize fails", () => {
      expect.assertions(1);

      fakeHostedFieldsInstance.tokenize.mockRejectedValue(
        new Error("bad happen")
      );
      jest.spyOn(cardView.model, "addPaymentMethod").mockImplementation();
  ...
  describe("field errors", () => {
    ...
    describe("showFieldError", () => {
      ...
      test("does not set hosted fields attributes on non hosted fields", () => {
    ...
    describe("hideFieldError", () => {
      ...
      test("does not remove hosted fields attributes on non hosted fields", () => {
        const fakeInput = document.createElement("input");
        const fakeGroup = document.createElement("div");
